...
const LandingHeader = ({path,setPath}:any) => {
  return (
    <div className='p-4 flex justify-between items-center'>
        <Link href='/' className='w-full flex gap-2 justify-start items-center'>
            ...
            <span className='font-semibold dark:text-white'>Bsamaritan</span>
        </Link>
        <NavigationMenu className='hidden md:block'>
            <NavigationMenuList className='gap-6'>
                    ...
                    <NavigationMenuContent>
                        <ul className='grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]'>
                            <li className='row-span-3'>
                                <span className='flex h-full w-full select-non flex-col justify-end rounded-md bg-gradient-to-b
                ...
                <NavigationMenuItem>
                    ...
                    className={cn({'dark:text-white': path === '#pricing','dark:text-white/40': path !== '#pricing',
                ...
                <NavigationMenuItem>
                    <NavigationMenuTrigger onClick={() => setPath('#features')}
                    className={cn({'dark:text-white': path === '#features','dark:text-white/40': path !== '#features',
                    'font-normal': true, 'text-xl': true,})}>
                        Features
                    </NavigationMenuTrigger>
                    <NavigationMenuContent>
                ...
                <NavigationMenuItem>
                    <NavigationMenuLink
                    className={cn(navigationMenuTriggerStyle(), {'dark:text-white': path === '#testimonials',
                        'dark:text-white/40': path !== '#testimonials','font-normal': true,'text-xl': true,})} >
                        Testimonial
                    </NavigationMenuLink>
                </NavigationMenuItem>
            </NavigationMenuList>
        ...
        <aside className='flex w-full gap-2 justify-end'>
            <Link href={'/auth/login'} className='text-white '>
                <Button variant='outline' className='p-2 text-center hidden sm:block'>Login</Button>
            </Link>
            <Link href={'/auth/login'} className='text-white'>
                <Button variant='default' className='p-2 text-center hidden sm:block'>Signup</Button>
            </Link>
        </aside>
    </div>
  )
    ...
    ({className, title, children, ...props},ref)=>{
        return <li>
            <NavigationMenuLink asChild>
                <a ref={ref} className={cn('group block select-none space-y-1 fonte-medium leading-none')} {...props}>
