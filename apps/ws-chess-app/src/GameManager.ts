...
import {GAME_OVER,INIT_GAME,JOIN_GAME,MOVE,OPPONENT_DISCONNECTED,JOIN_ROOM,GAME_JOINED,GAME_NOT_FOUND,GAME_ALERT,GAME_ADDED,
    GAME_ENDED,} from './messages';
        ...
        user.socket.on('message', async (data) => {
            ...
            if (message.type === INIT_GAME){
                if (this.pendingGameId){
                    ...
                    if (user.userId === game.player1UserId){
                        SocketManager.getInstance().broadcast(
                            ...
                            JSON.stringify({
                                type: GAME_ALERT,
                                payload:{
                                    message: ' Trying to connectwith yourself?',
                                }
                            })
                        );
                        return;
                    ...
                    SocketManager.getInstance().addUser(user, game.gameId);
                    SocketManager.getInstance().broadcast(
                        game.gameId, JSON.stringify({type: GAME_ADDED})
                    )
                }
            }
